...
public class CreateNewGameShould
{
   ...
   public void RaiseGameStartedEvent(APlayerId playerId, IShufflingService shuffleService)
   {
      ...
      var newGameEvent = game.Events.Should().ContainSingle(e => e is NewGameCreatedEvent).Subject.As<NewGameCreatedEvent>();

      var joinedEvent = game.Events.Should().ContainSingle(e => e is PlayerJoinedEvent).Subject.As<PlayerJoinedEvent>();
