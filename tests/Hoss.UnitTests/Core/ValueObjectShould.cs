...
public class ValueObjectShould
{
   ...
   {
      var properties = GetValueTypesFromAssembly().SelectMany(v => v.GetProperties());
      ...
      setMethods.ForEach
         (m =>
         {
            ...
            privateOrInit.Should().BeTrue($"{m.DeclaringType?.FullName} 👉 {m.Name} method should be private.");
         });
   }
   ...
   {
      var properties = GetValueTypesFromAssembly().SelectMany(v => v.GetProperties());
      var readOnlyProperties = properties.Where(p => p.SetMethod is null).ToList();

      readOnlyProperties.ForEach(p => p.CanWrite.Should().BeFalse($"{p.DeclaringType?.FullName} 👉 {p.Name} property should be readOnly."));
   }
   ...
   private static List<Type> GetValueTypesFromAssembly()
   {
      ...
      List<Type> types = AllTypes.From(assembly).ThatDeriveFrom<ValueObject>().ToList();
      return types;
   }
